...
```{r}
distribution_1990s <- ggplot(data=filter(rotten_tomatoes, decade=="1990s"), aes(x=audienceScore)) +
  geom_density() +
  labs(title="Distribution of Audience Scores for Films in the 1990s",
       x="Audience Scores",
       y="Density")

distribution_2010s <- ggplot(data=filter(rotten_tomatoes, decade=="2010s"), aes(x=audienceScore)) +
  geom_density() +
  labs(title="Distribution of Audience Scores for Films in the 2010s",
       x="Audience Scores",
       y="Density")

grid.arrange(distribution_1990s, distribution_2010s)

rotten_tomatoes %>%
  filter(decade == "1990s" | decade == "2010s") %>%
  group_by(decade) %>%
  summarise(avg_audience_score = mean(audienceScore),
            avg_critic_score = mean(critic_score),
            avg_runtimeMinutes = mean(runtimeMinutes, na.rm=T),
            prop_English = sum(originalLanguage=="English", na.rm=T) / n(),
            prop_Documentary = sum(Documentary, na.rm=T) / n(),
            prop_Horror = sum(Horror, na.rm=T) / n(),
            prop_Musical = sum(Musical, na.rm=T) / n(),
            prop_War = sum(War, na.rm=T) / n()
  ) %>%
  t()
```
...
```{r}
rotten_tomatoes_COVID <- rotten_tomatoes %>%
  filter(releaseDateTheaters >= 2016 & releaseDateTheaters <= 2023) %>%
  mutate(COVID = case_when(
    releaseDateTheaters < 2020 ~ "2016-2019",
    .default = "2020-2023"
  ))

ggplot(data=rotten_tomatoes_COVID, aes(x=audienceScore)) +
  geom_density() +
  facet_wrap(~COVID) +
  labs(title="Distribution of Audience Scores Before/After the COVID-19 Pandemic",
       x="Audience Scores",
       y="Density") 

ggplot(data=rotten_tomatoes_COVID, aes(y=audienceScore, x=critic_score)) +
  geom_point() +
  geom_smooth(method="loess", formula="y~x", se=F) +
  facet_wrap(~COVID) +
  labs(title="Audience Score vs. Critic Score Before/After COVID-19 Pandemic",
       y="Audience Score",
       x="Critic Score")
```
